/**
 * Study and implementation Worst Fit memory allocation methods
 */
...
class WorstFit{
    public static void main(String[] args) {
        ...
        for (int i=0; i<no_of_process; i++) {
            int max_diff = (int)Double.NEGATIVE_INFINITY;
            int max_index = -1;
            for(int j=0; j<no_of_blocks; j++){
                if(!blocksUsed[j] && processes[i] <= blocks[j] && (blocks[j] - processes[i])>max_diff){
                    max_diff = blocks[j] - processes[i];
                    max_index = j;
                }
            }
            if(max_index == -1) continue;
            blocksUsed[max_index] = true;
            assignedProcess[i] = max_index;
        }
